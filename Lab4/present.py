...
def genRoundKeys(key):
    round_keys = {}
    sub_mask = 0xf0000000000000000000
    xor_mask = 0x00000000000000
    for i in range(0, 8):
        print(bin(key))
        key = rol(key, 61, 80)
        print(bin(key))
        sub = (sBoxLayer(key & sub_mask) << 16)
        key = (key & ~sub_mask) | (sub & sub_mask)
        print(bin(i << 16))
...
def sBoxLayer(state):
    nibs = [(15 * (2 ** i) & state) // 2 ** i for i in range(0, 64, 4)]
    subs = [sbox[nib] << (i * 4) for i, nib in enumerate(nibs)]
    return reduce(or_, subs)


def pLayer(state):
    state = state >> 16
    bits = [((2 ** (63 - i)) & state) >> (63 - i) << (63 - p) for i, p in enumerate(pmt)]
    perm = reduce(or_, bits)
    return perm
...
if __name__ == "__main__":
    # Test Substitution Box
    key1 = 0x00000000000000000000
    test = 0xCCCCCCCCCCCCCCCC
    out = sBoxLayer(key1)
    assert out == test

    # Test Permutation Box
    key1 = 0x72000000000000000000
    test = 0x00008000C0008000
    out = pLayer(key1)
    assert out == test

    key1 = 0xf7200000000000000000
    ...
    for k in keysTest.keys():
        try:
            ...
            print(f"{keysTest[k]} != {keys[k]}")
            raise e
    # ===== End of custom test cases ===== #
